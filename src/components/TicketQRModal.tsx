...
import { generateCheckInQRData } from "@/services/ticketCheckInService";
...
const TicketQRModal: React.FC<TicketQRModalProps> = ({ isOpen, onClose, ticket }) => {
  ...
  const hiroExplorerUrl = ticket.mintTxId
    ? `https://explorer.hiro.so/txid/${ticket.mintTxId}?chain=${network}`
    : `https://explorer.hiro.so?chain=${network}`;

  // Generate check-in QR data if we have contract info
  let qrCodeData = hiroExplorerUrl;
  ...
  if (ticket.contractId && ticket.tokenId !== undefined) {
    ...
    if (contractAddress && contractName) {
      ...
      qrCodeData = generateCheckInQRData({
        contractAddress,
        contractName,
