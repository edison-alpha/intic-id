...
const Staking = () => {
  ...
  const calculateRewards = () => {
    if (!stakeAmount || isNaN(Number(stakeAmount))) return "0.00000000";
    const amount = Number(stakeAmount);
    const pool = stakingPools.find(p => p.id === selectedPool);
    const apy = parseFloat(pool?.apy || "0") / 100;
    return (amount * apy / 12).toFixed(8); // Monthly estimate in BTC format
  };
      ...
      <div className="px-4 pb-6 md:px-6 md:pb-8 max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#FE5C02] to-purple-600 flex items-center justify-center">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-8">
          <div className="bg-[#0A0A0A] border-2 border-gray-800 rounded-xl p-4 hover:border-[#FE5C02]/50 transition-all group">
            <div className="flex items-center gap-2 text-gray-400 mb-2">
              <Lock className="w-4 h-4 group-hover:text-[#FE5C02] transition-colors" />
              <span className="text-xs font-medium">Total Staked</span>
            </div>
            <div className="flex items-center gap-2 mb-1">
              <p className="text-2xl md:text-3xl font-bold text-white">{(userStats.totalStaked * 0.00001).toFixed(6)}</p>
              <img src="https://ipfs.io/ipfs/bafkreiffe46h5voimvulxm2s4ddszdm4uli4rwcvx34cgzz3xkfcc2hiwi" alt="sBTC" className="w-6 h-6" />
            </div>
            <p className="text-xs text-gray-500">≈ ${(userStats.totalStaked * 0.5).toFixed(2)} USD</p>
          </div>
          ...
          <div className="bg-[#0A0A0A] border-2 border-gray-800 rounded-xl p-4 hover:border-[#FE5C02]/50 transition-all group">
            <div className="flex items-center gap-2 text-gray-400 mb-2">
              <Gift className="w-4 h-4 group-hover:text-[#FE5C02] transition-colors" />
              <span className="text-xs font-medium">Rewards</span>
            </div>
            <div className="flex items-center gap-2 mb-1">
              <p className="text-2xl md:text-3xl font-bold text-[#FE5C02]">{(userStats.rewards * 0.00001).toFixed(8)}</p>
              <img src="https://ipfs.io/ipfs/bafkreiffe46h5voimvulxm2s4ddszdm4uli4rwcvx34cgzz3xkfcc2hiwi" alt="sBTC" className="w-6 h-6" />
            </div>
            <button className="text-xs text-[#FE5C02] hover:text-white font-medium flex items-center gap-1 group-hover:gap-2 transition-all">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-[#0A0A0A] border-2 border-gray-800 rounded-xl p-6">
              <div className="flex items-center justify-between mb-6">
                ...
                <Sparkles className="w-6 h-6 text-[#FE5C02]" />
              </div>
                  ...
                  return (
                    <div
                      ...
                      className={`relative bg-gradient-to-br ${pool.gradient} p-[1px] rounded-2xl cursor-pointer transition-all hover:scale-105 ${
                        isSelected ? "ring-2 ring-[#FE5C02] ring-offset-2 ring-offset-[#0A0A0A]" : ""
                      }`}
                    >
                      <div className="bg-[#0A0A0A] rounded-2xl p-5 h-full">
                        ...
                        {pool.popular ? (
                          <div className="flex items-center gap-1 mb-3">
                            <Flame className="w-3 h-3 text-[#FE5C02]" />
                            <span className="text-xs font-bold text-[#FE5C02]">{pool.badge}</span>
                          </div>
                        ) : (
                          <div className="flex items-center gap-1 mb-3">
                        ...
                        <div className="space-y-2 pt-3 border-t border-gray-800">
                          ...
                          <div className="flex items-center justify-between text-xs">
                            ...
                            <span className="text-white font-semibold">{pool.minStake} BTC</span>
                          </div>
                        </div>
                        
                        {/* Selected Indicator */}
                        {isSelected && (
                          <div className="mt-4 flex items-center gap-2 text-xs font-medium text-[#FE5C02]">
                            <div className="w-2 h-2 rounded-full bg-[#FE5C02] animate-pulse"></div>
                            Selected
                          </div>
                        )}
                      </div>
            ...
            <div className="bg-[#0A0A0A] border-2 border-gray-800 rounded-xl p-6">
              ...
              <div className="space-y-5">
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex items-center gap-1">
                      <span className="text-xs text-gray-500">Balance: {(userStats.walletBalance * 0.00001).toFixed(6)}</span>
                      <img src="https://ipfs.io/ipfs/bafkreiffe46h5voimvulxm2s4ddszdm4uli4rwcvx34cgzz3xkfcc2hiwi" alt="sBTC" className="w-4 h-4" />
                    </div>
                  </div>
                  <div className="relative bg-[#0A0A0A] border-2 border-gray-800 rounded-xl p-4 focus-within:border-[#FE5C02] transition-colors">
                    ...
                    <div className="flex items-center justify-between mt-3">
                      <span className="text-sm text-gray-500">≈ ${(Number(stakeAmount) * 0.5 || 0).toFixed(2)} USD</span>
                      <div className="flex gap-2">
                        <button 
                          onClick={() => setStakeAmount((userStats.walletBalance * 0.25).toString())}
                          className="px-3 py-1 text-xs font-medium bg-gray-800 hover:bg-gray-700 text-gray-300 rounded-lg transition-colors"
                        >
                          25%
                        </button>
                        <button 
                          onClick={() => setStakeAmount((userStats.walletBalance * 0.5).toString())}
                          className="px-3 py-1 text-xs font-medium bg-gray-800 hover:bg-gray-700 text-gray-300 rounded-lg transition-colors"
                        >
                          50%
                        </button>
                        <button 
                          onClick={() => setStakeAmount(userStats.walletBalance.toString())}
                          className="px-3 py-1 text-xs font-medium bg-[#FE5C02] hover:bg-[#E54F02] text-white rounded-lg transition-colors"
                        >
                          MAX
                        </button>
                      </div>
                ...
                <div className="bg-gradient-to-br from-purple-500/10 to-[#FE5C02]/10 border border-purple-500/20 rounded-xl p-5">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center gap-2">
                      <span className="text-3xl font-bold bg-gradient-to-r from-purple-400 to-[#FE5C02] bg-clip-text text-transparent">
                        {calculateRewards()}
                      </span>
                    ...
                    <div className="grid grid-cols-3 gap-3 pt-3 border-t border-purple-500/20">
                      <div>
                        ...
                        <div className="flex items-center gap-1">
                          <p className="text-sm font-semibold text-white">{(Number(calculateRewards()) / 30).toFixed(8)}</p>
                          <img src="https://ipfs.io/ipfs/bafkreiffe46h5voimvulxm2s4ddszdm4uli4rwcvx34cgzz3xkfcc2hiwi" alt="sBTC" className="w-4 h-4" />
                        </div>
                      </div>
                      <div>
                        ...
                        <div className="flex items-center gap-1">
                          <p className="text-sm font-semibold text-white">{(Number(calculateRewards()) * 12).toFixed(8)}</p>
                          <img src="https://ipfs.io/ipfs/bafkreiffe46h5voimvulxm2s4ddszdm4uli4rwcvx34cgzz3xkfcc2hiwi" alt="sBTC" className="w-4 h-4" />
                        </div>
                      </div>
                      <div>
                        <p className="text-xs text-gray-500 mb-1">USD Value</p>
                        <p className="text-sm font-semibold text-green-500">${(Number(calculateRewards()) * 50000).toFixed(2)}</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="w-full bg-[#FE5C02] hover:bg-[#E54F02] disabled:bg-gray-800 disabled:text-gray-600 text-white font-bold py-5 rounded-xl transition-all group"
                >
          ...
          <div className="space-y-6">
            ...
            <div className="bg-[#0A0A0A] border-2 border-gray-800 rounded-xl p-6 overflow-hidden relative">
              ...
              <div className="p-4 bg-[#FE5C02]/10 border-2 border-[#FE5C02]/20 rounded-lg">
                <div className="flex items-center justify-between mb-3">
                ...
                <div className="w-full bg-gray-800 rounded-full h-2.5 overflow-hidden">
                  <div className="bg-gradient-to-r from-yellow-500 to-orange-500 h-2.5 rounded-full animate-pulse" style={{ width: "100%" }}></div>
                </div>
                ...
                {tiers.map((tier) => (
                  <div
                    ...
                    className={`bg-black/30 border-2 rounded-lg p-4 transition-all hover:scale-[1.02] ${
                      tier.current ? "border-[#FE5C02]" : "border-gray-800"
                    }`}
                  >
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          {tier.current && (
                            <span className="text-xs bg-[#FE5C02] text-white px-2 py-0.5 rounded-full font-bold">
                              Active
                            </span>
                          )}
                        </div>
            ...
            {/* Info Card - Modern */}
            <div className="bg-[#FE5C02]/5 border-2 border-[#FE5C02]/20 rounded-xl p-6">
              <div className="flex items-start gap-3 mb-4">
                <div className="w-10 h-10 rounded-lg bg-[#FE5C02]/20 flex items-center justify-center flex-shrink-0">
                  <Info className="w-5 h-5 text-[#FE5C02]" />
                </div>
                <div>
