...
const Governance = () => {
  ...
  const userStaking = {
    totalStaked: 2500,
    tier: "Silver",
    votingPower: 2500, // 1 sBTC = 1 vote
    votesUsed: 1000,
    votesAvailable: 1500
  };
  ...
  const proposals = [
    {
      ...
      description: "Proposal to add Drake as the main headliner for the Summer Music Festival 2025. This would increase ticket prices by 5 sBTC but significantly boost event prestige.",
      category: "Lineup",
    ...
    },
    {
      ...
      description: "Due to potential scheduling conflicts, we propose moving the event from June 20 to June 22. All ticket holders will be notified and given refund options if they cannot attend the new date.",
      category: "Schedule",
    ...
    },
    {
      ...
      description: "Proposal to create an exclusive VIP lounge area for Gold and Platinum tier holders. This would include complimentary food, drinks, and networking opportunities.",
      category: "Venue",
    ...
    },
    {
      ...
      description: "Proposal to implement surge pricing during high-demand periods. Prices would increase by up to 25% but tier discounts would still apply. This helps prevent bots and scalpers.",
      category: "Pricing",
      event: "Platform-wide",
      status: "passed",
    ...
    },
    {
      ...
      description: "Community proposal to reduce the platform royalty fee on secondary sales from 5% to 3%. This would make resale more attractive while still supporting the platform.",
      category: "Economics",
      event: "Platform-wide",
      status: "rejected",
  ...
  const handleVote = (proposalId: number, voteType: 'yes' | 'no' | 'abstain') => {
    if (userStaking.votesAvailable <= 0) {
      toast.error("Not enough voting power! Stake more sBTC to gain votes.");
      return;
    }

    setVotes({ ...votes, [proposalId]: voteType });
    toast.success(`Vote submitted: ${voteType.toUpperCase()}`, {
      description: `Used ${userStaking.totalStaked} voting power`
    });
  };
  ...
  return (
    <AppLayout>
      <div className="px-4 pb-6 md:px-6 md:pb-6">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl md:text-4xl font-bold text-white mb-2">Governance</h1>
          <p className="text-gray-400">Vote on proposals and shape the future of INTIC events</p>
        </div>

        {/* Voting Power Card */}
        <div className="mb-8 p-6 bg-gradient-to-r from-[#FE5C02]/10 to-purple-600/10 border-2 border-[#FE5C02]/30 rounded-xl">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div>
              <div className="flex items-center gap-2 mb-2">
                <TrendingUp className="w-5 h-5 text-[#FE5C02]" />
                <p className="text-gray-400 text-sm">Total Staked</p>
              </div>
              <div className="flex items-center gap-2">
                <p className="text-2xl font-bold text-white">{userStaking.totalStaked}</p>
                <img src="https://ipfs.io/ipfs/bafkreiffe46h5voimvulxm2s4ddszdm4uli4rwcvx34cgzz3xkfcc2hiwi" alt="sBTC" className="w-6 h-6" />
                <span className="text-gray-400">sBTC</span>
              </div>
            </div>
        ...
        <div className="mb-8">
          ...
          <div className="space-y-4">
            {activeProposals.map((proposal) => {
              ...
              return (
                <div key={proposal.id} className="bg-[#0A0A0A] border-2 border-gray-800 rounded-xl p-6 hover:border-[#FE5C02]/50 transition-all">
                  {/* Header */}
